...
export const getRegisteredDogs = async (req, res) => {
  ...
  const query = {
    registeredBy: req.user._id || req.user.id,
  };
...
export const getAdoptedDogs = async (req, res) => {
  try {
    const dogs = await Dog.find({ adoptedBy: req.user._id || req.user.id });
    res.json({ dogs });
  } catch (error) {
    res.status(500).json({ message: "Failed to fetch adopted dogs", error: error.message });
  }
...
// Get available dogs for adoption (not adopted, not registered by current user)
export const getAvailableDogs = async (req, res) => {
  try {
    const dogs = await Dog.find({
      registrationStatus: "Available",
      adoptedBy: null,
      registeredBy: { $ne: req.user._id || req.user.id },
    });
    res.json({ dogs });
  } catch (error) {
    res.status(500).json({ message: "Failed to fetch available dogs", error: error.message });
...
export const adoptDog = async (req, res) => {
  try {
    ...
    if (dog.registeredBy && dog.registeredBy.toString() === userId.toString()) {
      return res.status(403).json({ message: "You cannot adopt a dog you registered" });
    }
...
// Remove a dog (registered or adopted by user)
export const removeDog = async (req, res) => {
  try {
    const dog = await Dog.findById(req.params.id);
    ...
    if (
      !dog.registeredBy.equals(userId) &&
      !(dog.adoptedBy && dog.adoptedBy.equals(userId))
    ) {
      return res.status(403).json({ message: "Not authorized to delete this dog" });
    }
